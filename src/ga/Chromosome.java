...
public class Chromosome implements Comparable<Chromosome> {
    ...
    public int conflicts2(ProblemSet problemSet){
        ...
        return overbooked*3
                ...
                +profSched.values().stream().mapToInt(Integer::intValue).sum()*1;
    }

    private int[] individualConflicts(ProblemSet problemSet){
        HashMap<Util.Tuple<Timeslot, Room>, Integer> roomUsage = new HashMap<>();
        HashMap<Util.Tuple<Timeslot, String>, Integer> profSched = new HashMap<>();
        int overbooked = 0;
        ...
        return new int[]{
                overbooked,
                roomUsage.values().stream().mapToInt(Integer::intValue).sum(),
                profSched.values().stream().mapToInt(Integer::intValue).sum()
        };
    }

    public double conflicts3(ProblemSet problemSet){
        ...
        for(var item : this.genes){
            ...
            for(int i = timeslot.hour; i < timeslot.hour + course.duration; i ++){
                var currTimeslot = new Timeslot(timeslot.day, i);
                profSched.compute(new Util.Tuple<>(currTimeslot, course.professor), (k, v) -> v==null?1:v+1);
                roomUsage.compute(new Util.Tuple<>(currTimeslot, room), (k, v) -> v==null?1:v+1);
            }
        }

//        var ob = overbooked*3;
        var ru = roomUsage.values().stream().filter(i -> i > 1).mapToInt(Integer::intValue).sum();
        var ps = profSched.values().stream().filter(i -> i > 1).mapToInt(Integer::intValue).sum();
//        if(ob >= ru && ob >= ps){
//            return ob + 1-1.0/(ru+ps+1) + ob+ru+ps;
//        }else if(ru >= ob && ru >= ps){
//            return ru + 1-1.0/(ob+ps+1) + ob+ru+ps;
//        }else{
//            return ps + 1-1.0/(ru+ob+1) + ob+ru+ps;
//        }
        return overbooked*3+ru*2+ps;
    }

    public int conflictsBounded(ProblemSet problemSet){
        int conflicts = 0;
        ...
        for(var item : this.genes){
            ...
            for(int i = timeslot.hour; i < timeslot.hour + course.duration; i ++){
                ...
                if(!problemSet.timeslots.contains(currTimeslot)) conflicts += 4;
            }
        }
    ...
    public int conflictsCustom(ProblemSet problemSet){
        int conflicts = 0;
        ...
        for(var item : this.genes){
            ...
            if(course.students > room.capacity) conflicts += 1;
            ...
            for(int i = timeslot.hour; i < timeslot.hour + course.duration; i ++){
                ...
                if(!problemSet.timeslots.contains(currTimeslot)) conflicts += 1;
            }
        }
    ...
    public double conflictsCustom33(ProblemSet problemSet){
        var meow = this.individualConflicts(problemSet);
        return meow[0] * Math.PI + meow[1] * Math.E + meow[2];
    }


    public int paretoDominates(Chromosome other, ProblemSet ps) {
        var t1 = this.individualConflicts(ps);
        var t2 = other.individualConflicts(ps);
    ...
    public int paretoDominates2(Chromosome other, ProblemSet ps) {
        var t1 = this.individualConflicts(ps);
        var t2 = other.individualConflicts(ps);
        int tracker = 0;
        for(int i = 0; i < t1.length; i ++){
//            if(t1[i] < t2[i]) tracker +=1;
//            if(t1[i] > t2[i]) tracker -=1;
            tracker += t2[i]*(3-i)-t1[i]*(3-i);
        }
        return Integer.compare(tracker, 0);
    }
